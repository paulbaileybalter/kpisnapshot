import { json, getSessionToken } from "../../_lib/auth.js";
...
export async function onRequestGet({ env }) {
  const latestMonth = await env.BALTER_KV.get(LATEST_KEY);
  const indexRaw = await env.BALTER_KV.get(INDEX_KEY);
  ...
  const snapRaw = await env.BALTER_KV.get(`data:month:${latestMonth}`);
...
export async function onRequestPost({ request, env }) {
  ...
  const existingRaw = await env.BALTER_KV.get(key);
  ...
  await env.BALTER_KV.put(key, JSON.stringify(snapshot));
  await env.BALTER_KV.put(LATEST_KEY, month);

  const indexRaw = await env.BALTER_KV.get(INDEX_KEY);
  const monthIndex = indexRaw ? JSON.parse(indexRaw) : [];
  if (!monthIndex.includes(month)) {
    monthIndex.push(month);
    await env.BALTER_KV.put(INDEX_KEY, JSON.stringify(monthIndex));
  }
